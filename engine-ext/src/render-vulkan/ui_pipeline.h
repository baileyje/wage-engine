...
//       UiPipeline(VulkanContext* context) : Pipeline(context) {}
...
//         vertShader.create(fs->read({{"resources/shader/font.vert.spv"}}, memory::Allocator::Temporary()));
//         Shader fragShader(&context->device, VK_SHADER_STAGE_FRAGMENT_BIT);
//         fragShader.create(fs->read({{"resources/shader/font.frag.spv"}}, memory::Allocator::Temporary()));
