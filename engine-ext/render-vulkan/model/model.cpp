...
namespace wage::render::vulkan {
  ...
  void Model::push(Device* device, CommandPool* commandPool, ModelPipeline* pipeline, int imageCount) {
    if (pushed) return;
    mesh->push(device, commandPool);
    createDescriptorPool(device, imageCount);
    textureAsset->texture.push(device, commandPool, pipeline->textureLayout, descriptorPool, imageCount);
    pushed = true;
  }
