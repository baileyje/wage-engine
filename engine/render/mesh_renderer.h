    ...
    public:
      ...
      void update(const ecs::SystemContext& context) {
        ...
        for (auto entity : manager->with({MeshComponent, TransformComponent, MaterialComponent})) {
          core::Core::Instance->get<Renderer>()->renderMesh(*entity.get<math::Transform>(TransformComponent), entity.get<Mesh>(MeshComponent), entity.get<Material>(MaterialComponent));
        }
      }
    };
